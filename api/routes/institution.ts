...
router.get('/:id/candidates', authenticateToken, (req: Request, res: Response): void => {
  try {
    const { id } = req.params
    if (req.user?.role === 'institution' && req.user.institutionId !== id) {
...
router.post('/:id/candidates', authenticateToken, (req: Request, res: Response): void => {
  try {
    const { id } = req.params
    if (req.user?.role === 'institution' && req.user.institutionId !== id) {
...
router.put('/:institutionId/candidates/:candidateId', authenticateToken, (req: Request, res: Response): void => {
  try {
...
router.delete('/:institutionId/candidates/:candidateId', authenticateToken, (req: Request, res: Response): void => {
  try {
    const { institutionId, candidateId } = req.params
    if (req.user?.role === 'institution' && req.user.institutionId !== institutionId) {
    ...
    const db = getDb()
    const hasRegistrations = db.prepare('SELECT COUNT(*) as count FROM registrations WHERE candidate_id = ?').get(candidateId) as any
