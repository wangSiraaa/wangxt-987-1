...
router.post('/', authenticateToken, requireRole('proctor', 'exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    ...
    `).get(seat_arrangement_id) as any
    if (!seat) {
    ...
    const proctor = db.prepare('SELECT id FROM proctors WHERE user_id = ?').get(req.user?.id) as any
    if (!proctor) {
...
router.post('/mark-absent', authenticateToken, requireRole('proctor', 'exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    ...
    const tx = db.transaction(() => {
      seat_arrangement_ids.forEach((id: string) => {
        const seat = db.prepare(`
...
router.get('/records/:scheduleId', authenticateToken, (req: Request, res: Response): void => {
  try {
...
router.post('/exception', authenticateToken, (req: Request, res: Response): void => {
  try {
    ...
    const reporterId = req.user?.role === 'proctor'
      ? (db.prepare('SELECT id FROM proctors WHERE user_id = ?').get(req.user?.id) as any)?.id
      : req.user?.id
    db.prepare(`
      INSERT INTO exception_audits (
        id, type, registration_id, candidate_id, schedule_id, seat_arrangement_id,
...
router.get('/exceptions', authenticateToken, (req: Request, res: Response): void => {
  try {
    const { status, type, candidate_id } = req.query as Record<string, string>
    const db = getDb()
    let sql = `
      SELECT e.*,
             c.name as candidate_name, c.id_card,
...
router.put('/exceptions/:id/handle', authenticateToken, requireRole('exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    const { id } = req.params
    ...
    const db = getDb()
    const now = dayjs().format('YYYY-MM-DD HH:mm:ss')
    const exception = db.prepare('SELECT * FROM exception_audits WHERE id = ?').get(id) as any
    if (!exception) {
...
router.post('/makeup', authenticateToken, requireRole('exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
...
router.get('/makeups', authenticateToken, (req: Request, res: Response): void => {
  try {
...
router.post('/score-unlock', authenticateToken, requireRole('exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    const { registration_id, reason } = req.body
    if (!registration_id || !reason) {
...
router.get('/score-unlocks', authenticateToken, requireRole('exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    const { status } = req.query as Record<string, string>
    const db = getDb()
    let sql = `
...
router.put('/score-unlocks/:id/approve', authenticateToken, requireRole('exam_admin', 'system'), (req: Request, res: Response): void => {
  try {
    const { id } = req.params
    const { status } = req.body
    const db = getDb()
    const now = dayjs().format('YYYY-MM-DD HH:mm:ss')
    const unlock = db.prepare('SELECT * FROM score_unlocks WHERE id = ?').get(id) as any
    if (!unlock) {
    ...
    const tx = db.transaction(() => {
      db.prepare(`
        ...
        WHERE id = ?
      `).run(targetStatus, req.user?.id, now, id)
    ...
    tx()
    res.json({ success: true, message: targetStatus === 'approved' ? '已批准成绩解锁' : '已驳回成绩解锁' })
  } catch (error) {
...
router.get('/master-data', authenticateToken, (_req: Request, res: Response): void => {
  try {
    const db = getDb()
    const skillLevels = db.prepare('SELECT * FROM skill_levels ORDER BY code').all().map((l: any) => ({
      ...l,
      value: l.id,
      label: l.name,
    }))
