...
router.get('/batches', authenticateToken, (req, res) => {
    try {
        const { status, skill_level_id, subject_id } = req.query;
        const db = getDb();
        let sql = `
      SELECT b.*, 
...
router.post('/batches', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
...
router.put('/batches/:id', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/', authenticateToken, (req, res) => {
    try {
        const { batch_id, exam_room_id, proctor_id, exam_date, status } = req.query;
        const db = getDb();
        let sql = `
      SELECT s.*, 
...
router.post('/validate', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { registration_ids, target, options } = req.body;
    ...
    `).all(...registration_ids);
        const result = scheduleValidator.validateAll(candidates, target, options || {});
...
router.post('/:id/assign', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
        ...
        const db = getDb();
        const schedule = db.prepare('SELECT * FROM exam_schedules WHERE id = ?').get(id);
        if (!schedule) {
            res.status(404).json({ success: false, error: '排考不存在' });
            return;
        }
...
router.post('/:id/expand', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
        ...
        const room = db.prepare('SELECT capacity FROM exam_rooms WHERE id = ?').get(schedule.exam_room_id);
...
router.post('/seat-adjust', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        ...
        const db = getDb();
        const oldSeat = db.prepare('SELECT * FROM seat_arrangements WHERE id = ?').get(seat_arrangement_id);
        if (!oldSeat) {
            res.status(404).json({ success: false, error: '座位安排不存在' });
            return;
        }
        const newSchedule = db.prepare('SELECT * FROM exam_schedules WHERE id = ?').get(new_schedule_id);
        if (!newSchedule) {
...
router.get('/:id/today', authenticateToken, requireRole('proctor', 'exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
