...
router.get('/', authenticateToken, (req, res) => {
    try {
        const { institution_id, status, payment_status, exam_status, subject_id, skill_level_id } = req.query;
        const db = getDb();
        let sql = `
      SELECT r.*, 
...
router.get('/pending', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const db = getDb();
...
router.post('/:id/freeze', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/unfreeze', authenticateToken, requireRole('exam_admin', 'system'), (req, res) => {
    try {
        const { id } = req.params;
        const db = getDb();
        const registration = db.prepare('SELECT * FROM registrations WHERE id = ?').get(id);
...
router.put('/:id/exam-status', authenticateToken, requireRole('exam_admin', 'system', 'proctor'), (req, res) => {
    try {
        const { id } = req.params;
